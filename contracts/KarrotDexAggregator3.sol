...
contract KarrotDexAggregator is Ownable {
    ...
    /* -------------------- RELAYER: Thor -------------------- */
    function requestThorSwap(address tokenIn, uint256 amountIn, string calldata chain, string calldata memo\1whenNotPaused nonReentrant external returns (bytes32 requestId)
    {
        _pull(IERC20(tokenIn), msg.sender, amountIn);
    ...
    function settleThorSwap(bytes32 requestId, address to, address tokenOut, uint256 amountOut, bytes calldata proof\1whenNotPaused nonReentrant external {
    ...
    function settleZKSwap(bytes32 requestId, address to, address tokenOut, uint256 amountOut, bytes calldata proof\1whenNotPaused nonReentrant external {
